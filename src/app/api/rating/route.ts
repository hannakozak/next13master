...
export async function POST(request: NextRequest) {
	...
	if (body && body.data && body.data.product.id) {
		...
		if (
			...
			productReviewsRating.reviewsConnection.edges.length > 0
		) {
			const total =
				productReviewsRating.reviewsConnection.edges.reduce(
					(acc, edge) => {
			...
			averageRating =
				total === 0
					...
					: Math.floor(
							total /
								productReviewsRating.reviewsConnection.aggregate
									.count,
					  );
		}
